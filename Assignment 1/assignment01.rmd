...
```{r}
pred_expost <- predict(fit1, data1)
```  

We can see the side-by-side comparison of actual vs. ex-post forecast:
  
```{r}
compare_ex_post <- data.frame(rAAPL[-1,], pred_expost)
compare_ex_post
```
  
We note that the forecast is not exactly accurate.
...
#ex_ante_forecast <- forecast(fit1, newdata=data.frame(x=c(.010, .015, .015), se.fit=TRUE))
```
